...
Jupyter notebooks working through the Anthropic Python SDK, part 3: built-in tools, extended
thinking, multimodal inputs, citations, prompt caching, and code execution.
...
| Path | Used by |
| --- | --- |
| `images/` | `50` — satellite property images (the notebook loads `prop7.png`; the others are alternates you can swap in) |
| `pdfs/earth.pdf` | `60`, `70` — source PDF |
| `streaming.csv` | `90` — churn dataset for the code execution sandbox |
| `outputs/` | Sample results, plus where `90` downloads files back from the sandbox |
| `.backups/` | Written by the `23` text editor tool before it edits a file (gitignored) |

Notebook `23` has Claude author `pie.py` and `test-pie.py` in the project root during the demo;
...
- The `23` text editor tool is sandboxed to `base_dir` (defaults to the notebook's working directory) and refuses paths that escape it.
